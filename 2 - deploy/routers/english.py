...
def init(model_path):
    ...
    tokenizer = ppnlp.transformers.ErnieTokenizer.from_pretrained(model_path)
...
def predict(text, predictor, tokenizer, label_map, max_seq_len=200):
...
@router.post("/")
def ipc_english_classifier(text: Text):
    if not text.data:
        return {'result': '请输入文本'}

    labels = predict(text.data.lower(), predictor, tokenizer, label_map)
